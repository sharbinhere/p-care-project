...
class CaretakerAuthController extends GetxController{
  ...
  CaretakerModel user = CaretakerModel(
    ...
    // Hardcoded for now, you can change based on user choice
  );
  await db.collection("CareTakers").doc(auth.currentUser?.uid).collection("Profile").add(user.toMap());

  // Add role to "Patiants" or "CareTakers" collection to track roles
  await db.collection('CareTakers').doc(auth.currentUser?.uid).set({
    'role': 'caretaker', // Change this dynamically if needed
  });
}
